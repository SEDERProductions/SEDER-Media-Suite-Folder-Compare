...
private slots:
    void exposesRowsAndRoles()
    {
        ...
        QCOMPARE(model.data(model.index(0, 1), Qt::DisplayRole).toString(), QStringLiteral("A001/clip.mov"));
        QCOMPARE(model.data(model.index(1, 0), CompareResultTableModel::StatusCodeRole).toInt(), CompareRow::Changed);
    }
    ...
    void filtersByOperationalState()
    {
        ...
        QCOMPARE(proxy.rowCount(), 1);
        QCOMPARE(proxy.data(proxy.index(0, 1), Qt::DisplayRole).toString(), QStringLiteral("folder"));
    }
